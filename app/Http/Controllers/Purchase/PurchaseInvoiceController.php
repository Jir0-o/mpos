...
class PurchaseInvoiceController extends Controller
{
    ...
    {
        return view('purchase/purchaseFormForMpos');
    }
    ...
    public function storePurchaseProductData(Request $request)
    {
        ...
        $count = TemporaryImei::where('product_id', $request->product_id)->count();
        $prod = Product::where('product_id', $product_id)->first();
        $scOne = SubCategoryOne::where('sc_one_id', $prod->sc_one_id)->first();
        if ($scOne->category_id == 1) {
            if ($count != $request->quantity) {
                return response()->json(
                    [
                        'quantity_error' => true,
                        'message' => "No of Imei should Equeal to quantity"
                    ]
                );
            }
        }
    ...
    public function storePurchaseForm(Request $request)
    {
        ...
        foreach ($purchase_temporary_items as $items) {
            ...
            $product_attr = TemporaryImei::where('product_id', $details->product_id)->get();
            if ($product_attr) {
                foreach ($product_attr as $data) {
                    $ProductAttribute = new ProductAttribute();
                    $ProductAttribute->product_id = $data->product_id;
                    $ProductAttribute->imei = $data->imei;
                    $ProductAttribute->ram = $data->ram;
                    $ProductAttribute->rom = $data->rom;
                    $ProductAttribute->color = $data->color;
                    $ProductAttribute->camera = $data->camera;
                    $ProductAttribute->purchase_details_id = $details->purchase_details_id;
    ...
    public function ajaxStoreImei(Request $request)
    {
        $count = TemporaryImei::where('product_id', $request->product_id)->count();
        if ($count >= $request->quantity) {
            return response()->json(
                [
                    'quantity_error' => true,
                    'message' => "No of Imei should not overdue quantity"
                ]
            );
        }

        $tempimei = TemporaryImei::where('product_id', $request->product_id)->get();
        foreach ($tempimei as $tempimeiData) {
            if ($tempimeiData->imei == $request->imei_barcode) {
                return response()->json(
                    [
                        'quantity_error' => true,
        ...
        $temporary_imei = new TemporaryImei();
        $temporary_imei->imei = $request->imei_barcode;
        $temporary_imei->product_id = $request->product_id;
        $temporary_imei->camera = $request->camera;
        $temporary_imei->rom = $request->rom;
        $temporary_imei->ram = $request->ram;
        $temporary_imei->color = $request->color;
        $temporary_imei->save();

        $data = TemporaryImei::where('product_id', $request->product_id)->get();
        return response()->json($data);
    }
    public function ajaxDelImei(Request $request)
    {
        $imei = TemporaryImei::find($request->key);
        $imei->delete();
